  ...
  return (
    ...
    >
      <div className={classes.pullCardWrapper} onClick={handleCardClick}>
        <img
          src={cardBack}
          className={classes.pullCardImage}
          alt="playing card back"
          style={yourTurn || isAdmin ? null : { filter: 'grayscale(100%)' }}
        />
        <span className={classes.pullCardRemaining}>
          {gameStatus.unplayedCards.length} Cards Left
        </span>
      </div>

      {gameStatus.lastPulledCard ? (
        <div className={classes.lastPulledWrapper}>
          <p className={classes.lastPulledCardOutcome}>
            {gameStatus.lastPulledCardOutcome}
          </p>
          <p className={classes.lastPulledCardInstruction}>
            {gameStatus.lastPulledCardInstruction}
          </p>

          <div className={classes.lastPulledCardImageWrapper}>
            <img
              src={cards[gameStatus.lastPulledCard]}
              alt={`card ${gameStatus.lastPulledCard}`}
              className={classes.lastPulledCardImage}
            />
            <div
              className={classes.playAgainWrapper}
              style={gameOver && isAdmin ? { display: 'block' } : null}
            >
              ...
              >
                <span role="img" aria-label="beer glasses">
                  &#127867;
                </span>
                Let's Go
              </button>
      ...
      <div className={classes.playerTurnWrapper}>
        <span className={classes.playerTurn}>
          {gameOver
            ? 'Game Over!'
            : `${gameStatus.players[gameStatus.turnIndex].player_name}'s Turn`}
        </span>
      </div>
